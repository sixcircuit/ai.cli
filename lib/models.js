...
module.exports = function(opts){
   ...
   model_class.prototype.get_warnings =  function({ stats }){
      ...
      if(stats.input.token_length > model_limit){
         warnings.push(`the model may lose track of facts in the middle of the context window. your number of input tokens: ${stats.input.token_length} is greater than the warning threshold for this model: ${model_limit}. the total model context length is: ${model_total}. you used ${ _.format.percent(stats.input.token_length / model_total, 1) } of the total context. the warning threshold is ${ _.format.percent(model_limit / model_total, 1) } of the total context.`);
      }
