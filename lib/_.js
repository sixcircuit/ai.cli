...
_.warn = function(...str){
   process.stderr.write("WARNING: " + str.join() + "\n");
};
...
_.resolve_alias = function(lookup, alias, limit = 10){
   let cursor = alias;

   let loop = 0;
   do{
      if(loop++ > limit){ _.fatal(`alias "${ alias }" exceeded loop max: ${ limit }.`); }
      if(lookup[cursor] === undefined || lookup[cursor] === cursor){ return(cursor);  }
