...
text_wrapper_class.prototype.stream = function({ chunk }){
   ...
   function _handle_word(word){
      ...
      }else if(self._line_len === 0){
         start_new_line();
      }

      // Check if the word fits in the current line
      if(self._line_len + word.length > n_wrap){
         out += "\n";
         self._line_len = 0;
         start_new_line();
      }

      if(self._line_len > n_gutter){
         out += " "
         self._line_len += 1;
      }

      out += word;
      self._line_len += word.length;
   ...
   _.each.s(words, function(word){ out += handle_word(word); });
