...
convo_class.prototype.tokenize = function(){
   ...
   _.each.s(this._messages, function(message){
      ...
      messages.push(_.merge(message, {
         tokens, token_length,
         price_in: model.price().tokens_in(token_length),
         price_out: model.price().tokens_out(token_length),
      }))
   });

   tokenized.price_in = model.price().tokens_in(tokenized.token_length);
   tokenized.price_out = model.price().tokens_out(tokenized.token_length);
