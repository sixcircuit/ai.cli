...
ai_class._load_config = function(){

   const fs = require('fs');

   const config = _.merge(require(this.paths().config_root("config.js")));

   const builtin_root = this.paths().src_root("./builtin/");
...
ai_class.load = function(){ return(new ai_class(ai_class._load_config())); };

ai_class.paths = function(){
   return({
      src_root: function(...path){ return _.path.resolve(__dirname + "/../" + path.join("/")); },
      config_root: function(...path){ return _.path.join(process.env.HOME, ".config/ai", ...path); }
   });
};
...
ai_class.prototype.paths = function(){ return ai_class.paths(); };
...
module.exports = _.assign(function(opts){ return(new ai_class(opts)); }, ai_class);
