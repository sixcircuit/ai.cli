...
console_class.prototype.flush = function({ color, new_line }){
   return this._out(color, this._buffer.stream({ flush: true, new_line }));
};

console_class.prototype.once = function({ color, chunk, new_line }){
   return this._out(color, this._buffer.once({ chunk, new_line }));
};
